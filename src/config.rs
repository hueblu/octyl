...
pub struct KeyMap(pub HashMap<KeyEvent, Box<dyn Action>>);
        ...
        D: Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for KeyMapVisitor {
            ...
            fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
                formatter.write_str(
                    "a keymap with string representation of KeyEvent as key and Action as value",
                )
            }
                ...
                M: MapAccess<'de>,
            {
                ...
                while let Some((key_str, action)) =
                    access.next_entry::<String, Box<dyn Action>>()?
                {
...
impl Config {
    pub fn new(
        ...
        builder = builder.add_source(
            config::File::from(config_dir.join("config.toml"))
